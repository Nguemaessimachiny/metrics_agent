...
# procps fournit la commande "uptime", utilisée par app/collector.py (service agent)
RUN apt-get update && apt-get install -y --no-install-recommends procps \
    && rm -rf /var/lib/apt/lists/*

# C'est le dossier de travail pour l'image finale
WORKDIR /app
...
# HEALTHCHECK permet à Docker de vérifier automatiquement que l'API fonctionne
# en testant l'endpoint /health toutes les 30 secondes.
